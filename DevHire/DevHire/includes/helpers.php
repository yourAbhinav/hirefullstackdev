...
function getSiteName(string $default = 'DevHire'): string
{
    ...
    if (is_file($cacheFile)) {
        ...
        if (is_array($cachedData)) {
            $cachedName = trim((string) ($cachedData['site_name'] ?? ''));
            if ($cachedName !== '') {
                $resolved = true;
                return $cachedName;
            }
    ...
    $siteName = trim((string) ($result['setting_value'] ?? ''));
    $cachedName = $siteName !== '' ? $siteName : $default;
    $resolved = true;

    if ($cachedName !== $default) {
        $cachePayload = json_encode(['site_name' => $cachedName], JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
