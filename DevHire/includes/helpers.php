...
function isMaintenanceModeRequestAllowed(): bool
{
    ...
    return str_contains($script, '/admin/')
        || str_contains($script, '/auth/admin')
        || str_contains($script, '/assets/')
        || str_contains($script, '/robots.txt')
        || str_contains($script, '/sitemap.xml');
}
...
function enforceMaintenanceMode(mysqli $conn): void
{
    ...
    if (isAdminLoggedIn() || isMaintenanceModeRequestAllowed()) {
        return;
    }
...
function refreshSiteNameCache(string $siteName): void
{
    ...
    $cacheFile = __DIR__ . '/../config/site-settings.cache.json';
...
function getSiteName(string $default = 'DevHire'): string
{
    ...
    if (is_file($cacheFile)) {
        ...
        if (is_array($cachedData)) {
            $cachedName = trim((string) ($cachedData['site_name'] ?? $cachedData['site_title'] ?? ''));
            if ($cachedName !== '') {
                $siteName = $cachedName;
                $resolved = true;
                return $siteName;
    ...
    foreach (['site_name', 'site_title'] as $key) {
        $stmt->bind_param('s', $key);
        $stmt->execute();
        $result = $stmt->get_result()->fetch_assoc();
        $candidate = trim((string) ($result['setting_value'] ?? ''));
    ...
    $siteName = $siteName !== null && $siteName !== '' ? $siteName : $default;
    $resolved = true;
...
/**
 * Load the generated images manifest if available.
 * Returns an associative array mapping original filenames to generated webp/srcset info.
 */
function loadImageManifest(): array
{
    $manifestFile = __DIR__ . '/../assets/images/generated/images-manifest.json';
...
/**
 * Render a responsive <picture> element using the manifest generated by `convert-images.js`.
 * - $originalFilename: filename as placed in `assets/images/` (e.g. "hero.jpg").
 * - $alt: alt text.
 * - $class: optional classes for the <img> fallback.
 * - $sizes: optional sizes attribute for the source/img elements.
 * - $loading: optional loading mode, default lazy.
 */
function renderResponsiveImage(string $originalFilename, string $alt = '', string $class = '', string $sizes = '', string $loading = 'lazy'): string
{
    $manifest = loadImageManifest();
    $entry = $manifest[$originalFilename] ?? null;

    if (!is_array($entry)) {
        // Fallback to original path if no generated assets available
        $src = appUrl('assets/images/' . ltrim($originalFilename, '/'));
        $attrs = trim(($class !== '' ? ' class="' . escape($class) . '"' : '') . ($sizes !== '' ? ' sizes="' . escape($sizes) . '"' : ''));
        return '<img loading="' . escape($loading) . '" decoding="async" src="' . escape($src) . '" alt="' . escape($alt) . '"' . $attrs . '>';
    }

    // Build srcset string for webp
    $webpSrcset = [];
    foreach ($entry['srcset'] as $s) {
        if (!empty($s['src']) && !empty($s['width'])) {
            $webpSrcset[] = escape(appUrl($s['src'])) . ' ' . (int) $s['width'] . 'w';
        }
    }

    $webpSrcsetAttr = implode(', ', $webpSrcset);
    $fallbackSrc = appUrl($entry['webp'] ?? $entry['original']);

    $classAttr = $class !== '' ? ' class="' . escape($class) . '"' : '';
    $sizesAttr = $sizes !== '' ? ' sizes="' . escape($sizes) . '"' : '';

    $html = '<picture>';
    $html .= '<source type="image/webp" srcset="' . $webpSrcsetAttr . '"' . $sizesAttr . '>'; 
    $html .= '<img loading="' . escape($loading) . '" decoding="async" src="' . escape($fallbackSrc) . '" alt="' . escape($alt) . '"' . $classAttr . $sizesAttr . '>';
    $html .= '</picture>';
