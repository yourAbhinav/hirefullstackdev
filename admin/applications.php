                ...
                <?php else: ?>
                    <?php foreach ($applications as $app): ?>
                        <tr>
                            ...
                            <td>
                                <div class="job-info">
                                    <div class="job-title"><?= htmlspecialchars($app['job_title'] ?? 'N/A') ?></div>
                                    <div class="job-role"><?= ucfirst($app['user_role'] ?? 'developer') ?></div>
                                </div>
                            </td>
                            <td>
                                <div class="date-info">
                                    <div><?= date('M j, Y', strtotime($app['created_at'])) ?></div>
                                    <div class="time-ago"><?= time_elapsed_string($app['created_at']) ?></div>
                                </div>
                            </td>
                            <td>
                            ...
                            <td>
                                <?php if ($app['resume_path']): ?>
                                    <button class="btn-icon btn-icon-sm" onclick="viewResume(<?= $app['id'] ?>)" title="View Resume">
                            ...
                            <td>
                                <div class="action-buttons">
                                    ...
                                    <button class="btn-icon" onclick="openStatusModal(<?= $app['id'] ?>)" title="Change Status">
                                        <i class="fas fa-edit"></i>
                                    </button>
                                    <button class="btn-icon" onclick="addNote(<?= $app['id'] ?>)" title="Add Note">
                                        <i class="fas fa-sticky-note"></i>
                                    </button>
                                    <button class="btn-icon btn-icon-danger" onclick="deleteApplication(<?= $app['id'] ?>)" title="Delete">
...
document.getElementById('statusForm').addEventListener('submit', function(e) {
    ...
    fetch('<?= appUrl('admin/api/application_api.php') ?>', {
        method: 'POST',
...
function addNote(appId) {
    ...
    if (note) {
        fetch('<?= appUrl('admin/api/application_api.php') ?>', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ action: 'add_note', application_id: appId, note })
        })
        .then(response => response.json())
        .then(data => {
            if (data.success) {
...
function deleteApplication(appId) {
    ...
    fetch('<?= appUrl('admin/api/application_api.php') ?>', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'delete', application_id: appId })
    })
    .then(response => response.json())
    .then(data => {
        if (data.success) {
...
function applyBulkAction() {
    ...
    fetch('<?= appUrl('admin/api/application_api.php') ?>', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: `bulk_${action}`, application_ids: selected })
    })
    .then(response => response.json())
    .then(data => {
        if (data.success) {
